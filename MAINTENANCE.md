...
`upstream/main` is a protected mirror. The sync workflow leaves it unchanged while an actualization PR is open.
...
8. Run `Prepare actualization` with that head branch. It opens the Draft PR against the current `upstream/main` and records the old `main` SHA in an automation marker.
9. After all normal PR checks pass, comment `/promote`. Promotion verifies that both `main` and `upstream/main` still have the recorded PR bases, validates the candidate against `upstream/main`, force-updates `main`, and closes the PR.
