...
`upstream/main` is a protected mirror. Actualization PRs use an immutable `upstream/main-<old-main-sha>` snapshot so the mirror can keep moving while a PR is reviewed.
...
8. Run `Prepare actualization` with that head branch. It creates `upstream/main-<old-main-sha>` at the current mirror and opens the Draft PR against that immutable snapshot.
9. After all normal PR checks pass, comment `/promote`. Promotion verifies that `main` still equals the SHA encoded in the snapshot name, validates the candidate against the snapshot, force-updates `main`, closes the PR, and deletes both temporary refs.

If `main` moves before promotion, refresh the actualization. Promotion uses a lease and refuses a stale snapshot.
