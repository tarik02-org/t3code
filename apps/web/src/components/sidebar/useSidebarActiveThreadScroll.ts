...
export function useSidebarActiveThreadScroll(input: {
  readonly hasThreadRoute: boolean;
  ...
  useLayoutEffect(() => {
    ...
    if (!initialScrollPendingRef.current && !sidebarBecameVisible && !routeThreadChanged) {
      sidebarWasVisibleRef.current = true;
    ...
    activeThread.scrollIntoView({
      behavior:
        initialScrollPendingRef.current || sidebarBecameVisible || prefersReducedMotion
          ? "instant"
          : "smooth",
      block: "center",
    });
    ...
    initialScrollPendingRef.current = false;
    sidebarNavigationThreadKeyRef.current = null;
  }, [hasThreadRoute, prefersReducedMotion, routeThreadKey, sidebarIsVisible]);
