...
export function resolveTimelineIsAtEnd(state: TimelineEndState | undefined): boolean | undefined {
  ...
  // the same amount of viewport, so the inset cancels.
...
/** Keep promptless provider restarts in one visual response until settled. */
...
function deriveTurnFolds(input: {
  ...
  for (const [turnId, group] of groupsByTurnId) {
    ...
    for (const [index, entry] of group.entries.entries()) {
      ...
      if (index > terminalEntryIndex) continue;
...
function attachTrailingToolGroupsToAssistant(
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (metaRow) result.push(metaRow);
  }
  return result;
}
    ...
    if (timelineEntry.kind === "work") {
      ...
      if (visibleGroupedEntries.length > 0) {
        ...
        if (activeInProgressToolEntries.length > 0) {
          ...
          (!input.isWorking ||
            visibleGroupedEntries[0]!.command !== undefined ||
            visibleGroupedEntries[0]!.toolTitle !== undefined)
        ) {
