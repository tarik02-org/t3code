...
describe("deriveTurnPlans", () => {
  it("keeps one stable row per turn with the latest snapshot", () => {
    const turnPlans = deriveTurnPlans([
      ...
      makeActivity({
        ...
        tone: "info",
        turnId: "turn-2",
        payload: { plan: [{ step: "Ship it", status: "pending" }] },
      }),
    ]);
    ...
    expect(turnPlans[0]).toMatchObject({
      ...
      createdAt: "2026-02-23T00:00:01.000Z",
      turnId: "turn-1",
      plan: { steps: [{ step: "Inspect code", status: "completed" }] },
    });
    expect(turnPlans[1]).toMatchObject({
      id: "turn-plan:turn-2",
      turnId: "turn-2",
      plan: { steps: [{ step: "Ship it", status: "pending" }] },
    });
  });

  it("drops a turn row when a later snapshot clears its plan", () => {
    expect(
      deriveTurnPlans([
        makeActivity({
          id: "plan-set",
...
describe("deriveTimelineEntries", () => {
  ...
  it("keeps inline turn plans in chronological order", () => {
    const entries = deriveTimelineEntries(
      [],
      [],
      [],
    ...
    expect(entries).toEqual([
      {
        ...
        kind: "turn-plan",
        createdAt: "2026-02-23T00:00:02.000Z",
        turnPlan: expect.objectContaining({ id: "turn-plan:turn-1" }),
      },
    ]);
  });
});
