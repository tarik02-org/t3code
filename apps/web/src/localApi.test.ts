...
describe("LocalApi", () => {
  ...
  it("delegates host capabilities and persistence to the desktop bridge", async () => {
    ...
    const nativeClientSettings = {
      ...DEFAULT_CLIENT_SETTINGS,
      contextMenuStyle: "native",
    };
    const getClientSettings = vi.fn().mockResolvedValue(nativeClientSettings);
    ...
    await expect(api.persistence.getClientSettings()).resolves.toEqual(nativeClientSettings);
    ...
    expect(getClientSettings).toHaveBeenCalledTimes(2);
