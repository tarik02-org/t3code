  ...
  const observedSpawner = ChildProcessSpawner.make((command) =>
    Effect.gen(function* () {
      ...
      const environment: NodeJS.ProcessEnv = {
        ...(command.options.env ?? {}),
        // Provider instance environment deliberately strips managed T3_* keys.
        // Re-add only the mock-agent controls at this test-only spawn seam.
        T3_ACP_ANTIGRAVITY: "1",
        T3_ACP_REQUEST_LOG_PATH: requestLog,
      };
      const handle = yield* spawner.spawn({
        ...command,
        options: { ...command.options, env: environment },
      });
      launches.push({
