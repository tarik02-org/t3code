import {
  ...
  type OrchestrationV2VisibleTurnItemPage,
...
export interface OrchestratorV2Shape {
  ...
  }) => Effect.Effect<OrchestrationV2VisibleTurnItemPage, OrchestratorV2Error>;
...
const makeOrchestrator = Effect.fn("orchestrationV2.Orchestrator.layer")(function* () {
  ...
  const getProjectionWithPendingEvents = (
    ...
    Effect.gen(function* () {
      ...
      const stored = yield* Effect.option(projectionStore.getOperationalProjection(threadId, 500));
  ...
  const startNextQueuedRun = (threadId: ThreadId) =>
    Effect.gen(function* () {
      const projection = yield* projectionStore.getOperationalProjection(threadId, 1);
      if (
  ...
  const resumeQueuedRuns = Effect.gen(function* () {
    ...
    for (const thread of shell.threads) {
      const resumedThread = yield* Effect.gen(function* () {
        const projection = yield* projectionStore.getOperationalProjection(thread.id, 1);
  ...
  const dispatchThreadMutation = Effect.fn("orchestrationV2.dispatch.threadMutation")(function* (
    ...
    const projection = yield* projectionStore.getOperationalProjection(command.threadId, 1).pipe(
      Effect.mapError(
        (cause) =>
  ...
  const dispatchProviderSessionDetach = Effect.fn("orchestrationV2.dispatch.providerSessionDetach")(
    function* (
      ...
      const projection = yield* projectionStore
        .getOperationalProjection(command.threadId, 1)
        .pipe(
          Effect.mapError(
  ...
  const dispatchDelegatedTaskRequest = Effect.fn("orchestrationV2.dispatch.delegatedTaskRequest")(
    function* (
      ...
      const parentProjection = yield* projectionStore
        .getOperationalProjection(command.parentThreadId, 1)
        .pipe(
          Effect.mapError(
            (cause) =>
              new OrchestratorProjectionError({
  ...
  const dispatchCreatedThreadRecord = Effect.fn("orchestrationV2.dispatch.createdThreadRecord")(
    function* (
      ...
      const targetProjection = yield* projectionStore
        .getOperationalProjection(command.targetThreadId, 1)
        .pipe(
          Effect.mapError(
            (cause) =>
              new OrchestratorProjectionError({
  ...
  const dispatchRuntimeRequestRespond = (
    ...
    Effect.gen(function* () {
      const projection = yield* projectionStore
        .getOperationalProjection(command.threadId, 500)
        .pipe(
  ...
  const loadProjectionForCommand = (command: OrchestrationV2Command) =>
    projectionStore
      .getOperationalProjection(commandThreadId(command), 500)
      .pipe(
        Effect.mapError(
  ...
  const appOwnedSubagentParentThreadId = (childThreadId: ThreadId) =>
    Effect.gen(function* () {
      const childProjection = yield* projectionStore.getOperationalProjection(childThreadId, 1);
  ...
  const finalizeAppOwnedSubagent = (childThreadId: ThreadId) =>
    Effect.gen(function* () {
      let childProjection = yield* projectionStore.getOperationalProjection(childThreadId, 1);
      ...
      const parentProjection = yield* projectionStore.getOperationalProjection(parentThreadId, 1);
      const task = parentProjection.subagents.find(
        (candidate) =>
      ...
      if ((task.completionWake ?? "settled_only") === "settled_only") {
        ...
        if (hasLiveRun(yield* projectionStore.getOperationalProjection(parentThreadId, 1))) {
          return;
        }
      }
      yield* continuationRequests.offer(
